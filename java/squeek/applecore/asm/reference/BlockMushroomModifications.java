	...
	public void updateTick(World world, BlockPos pos, IBlockState state, Random rand)
	{
		...
		if (allowGrowthResult == Result.ALLOW || (allowGrowthResult == Result.DEFAULT && rand.nextInt(25) == 0))
		{
			if (rand.nextInt(25) == 0)
			{
				...
				BlockPos pos1 = pos.add(rand.nextInt(3) - 1, rand.nextInt(2) - rand.nextInt(2), rand.nextInt(3) - 1);

				for (int k = 0; k < j; ++k)
				{
					if (world.isAirBlock(pos1) && this.canBlockStay(world, pos1, this.getDefaultState()))
					{
						pos = pos1;
					}

					pos1 = pos.add(rand.nextInt(3) - 1, rand.nextInt(2) - rand.nextInt(2), rand.nextInt(3) - 1);
				}

				if (world.isAirBlock(pos1) && this.canBlockStay(world, pos1, this.getDefaultState()))
				{
					world.setBlockState(pos1, this.getDefaultState(), 2);
				}
			}
			// added line
